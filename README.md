...
In order to use the basic functionality of our gem, you only need two steps:
 1. include `JumpIn` in the main controller that takes care of authentication in your app (e.g. `ApplicationController`),
 1. specify strategies you want to use in the main controller of the part of your application that is going to use them
 (app/API/whole application - it can be the `ApplicationController` as well).

Most of the time it will look as follows:
```
...
In order to use JumpIn you need to include `JumpIn` in the main controller that takes care of authentication in your app,
...
```
$ rails generate jump_in:configuration
```
which will create `config/initializers/jump_in/jump_in.rb` for you. You can set custom values there.
...
## Authentication
This module provides complex methods: `authenticate_and_login`(alias: `jump_in`), `login` and `jump_out`, two helper methods: `current_user` and `logged_in?`,
as well as extracted methods to be used according to your own preferences.

```
- authenticate_and_login(user:, by_cookies:false, **auth_params) # alias: jump_in
```
it authenticates object and logs it in (using `login` method). The authentication and login strategies are chosen
...
`Login` method sets:
- `session` if `:session` strategy has been chosen and `by_cookies` is not passed: `login(user: @student) unless logged_in?`,
...
- jump_out(**opts)
```
logs user out (it clears session or cookies depending on the previous choice of login strategy).
No arguments are required for deleting session and cookies.

Optional parameters might be used for additional strategies.
For example if you're using `LastLoggedOut` strategy, you'll need to pass `user` here, e.g. `jump_out(user: @student)`.
If you'll need another params for your custom strategy, you'll need to pass all the necessary params here.
...
In order to use a strategies (default and/or additional) you need to specify them in the main controller of the part of your application
that is going to use them (app/API/whole application - it will quite often be the `ApplicationController`), e.g.:
 ```
class ApplicationController < ActionController::Base
  jumpin_use :session, :by_password
...
#### Cookies
It is being used if `:cookies` is specified in `jumpin_use`.
For `authenticate_and_login`(/`jump_in`) and `login` methods you also need to pass `by_cookies:true`.
It adds the following methods to your controller:
- `set_user_cookies(user:)` - sets user's `class` & `id` in `cookies.signed` (set's cookies called by the calling methods in the following way: `set_user_cookies(user: @student, opts: { by_cookies: true } )`);
...
#### ByPassword
JumpIn authentication by password uses `has_secure_password`'s authenticate method. Therefore you need to add column `password_digest`
to your model's tabel and add: `has_secure_password` in the model to be authenticated. For example:
```
...
In order to log in with token you need to find (or create) the object using token and then simply use `login` method as described above. For example:
```
@student = Student.find(token: params[:token])
if @student
  login(user: @student) # or login(user: @student, by_cookies: true)
else
  ...
...
There are following additional strategies at the moment: `LastLoggedIn`, `LastLoggedOut`, `LoginsCounter`.
In order to use them you need to run generators and specify them in `jumpin_use` method in your controller.

- `LastLoggedIn`: `$ rails generate jump_in:last_logged_in` - this strategy keeps track of last login datetime for you,
...
The best way to start writing your own strategy is to generate our template by means of:
```
$ rails generate jump_in:custom_strategy
```
this will generate `app/jumpin/jumpin_custom_strategy.rb` for you.

```
module JumpIn::YourModuleName
...
If you add an authentication method (that goes to `get_authenticated_user:` group) you might need to add a custom `def authenticate()` method
to the model to be authenticated, e.g.:
```
...
```
Your custom authentication method must take two params: `user` and `auth_params` - which is a hash of keyword arguments specific for your strategy.
...
(in order to use the proper authentication strategy while iterating through all of the available ones)
```
def your_get_authenticated_user_method_name(user:, auth_params:)
  return nil unless auth_params.keys == [your_keyword_argument(s)] # e.g. [:password, :token, :answer]
